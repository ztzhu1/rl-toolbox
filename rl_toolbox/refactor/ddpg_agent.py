...
from torchrl.modules import Actor, ActorCriticWrapper, DdpgMlpActor, ValueOperator

from rl_toolbox.utils.backend import check_notebook, get_device
from rl_toolbox.utils.cp_utils import search_cp_file_name
...
class DDPGAgent:
    ...
    def _init_actor(self):
        ...
        actor = mlp(sizes, getattr(nn, cfg["activation"]), nn.Tanh)
    ...
    def _init_critic(self):
        ...
        critic = mlp(sizes, getattr(nn, cfg["activation"]), nn.Tanh)
        critic = Actor(actor, in_keys=["observation"], out_keys=["action"])
